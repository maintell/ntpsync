...
func main() {
	...
	ntime := getremotetime()
	ts := ntime.Format(timeLayoutStr) //time to string
	fmt.Print(ts)
...
func getremotetime() time.Time {
	var host string
	// using hong kong server list
	flag.StringVar(&host, "e", "hk.pool.ntp.org:123", "NTP host")
	flag.Parse()
	...
	case "windows":
		{
			...
			if err1 != nil && err2 != nil {
				glog.Info("sync datetime failed: please run as administrators")
				return false
			}
			return true
	...
	case "linux":
		{
			...
			if err1 != nil {
				glog.Info("sync datetime failed:", err1.Error())
				return false
			}
			return true
		}
